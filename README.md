...
<details>
  ...
  <pre><code class="language-sh">git config --global alias.branchdiff '!f() { base="$1"; if [ -z "$base" ]; then base=$(git symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null); if [ -z "$base" ]; then remote_head=$(git remote show origin 2>/dev/null | sed -n "/HEAD branch/s/.*: //p"); if [ -n "$remote_head" ]; then base="origin/$remote_head"; else echo "Could not determine default branch; pass one explicitly (git bd <branch>)" >&2; return 1; fi; fi; fi; if ! git rev-parse --verify -q "$base" >/dev/null; then echo "Base ref '"'"'$base'"'"' does not exist" >&2; return 1; fi; current=$(git symbolic-ref --short HEAD 2>/dev/null); cap=20; max=0; fifo1=$(mktemp -u); mkfifo "$fifo1"; git for-each-ref --sort=refname --format="%(refname:short)" refs/heads/ > "$fifo1" & while read -r branch; do [ ${#branch} -gt "$max" ] && max=${#branch}; done < "$fifo1"; rm -f "$fifo1"; fifo2=$(mktemp -u); mkfifo "$fifo2"; git for-each-ref --sort=refname --format="%(refname:short)" refs/heads/ > "$fifo2" & while read -r branch; do [ "$branch" = "$base" ] && continue; counts=$(git rev-list --left-right --count "$branch"..."$base" 2>/dev/null); [ -z "$counts" ] && continue; set -- $counts; ahead=$1; behind=$2; alen=$ahead; [ "$alen" -gt "$cap" ] && alen=$cap; blen=$behind; [ "$blen" -gt "$cap" ] && blen=$cap; abar=$(printf "%${alen}s" "" | tr " " "+"); bbar=$(printf "%${blen}s" "" | tr " " "-"); [ "$ahead" -gt "$cap" ] && abar="${abar}»"; [ "$behind" -gt "$cap" ] && bbar="${bbar}»"; padded=$(printf "%-*s" "$max" "$branch"); if [ "$branch" = "$current" ]; then marker="* "; name="\033[1;36m${padded}\033[0m"; else marker="  "; name="$padded"; fi; printf "%s%b \033[32m%s\033[0m\033[31m%s\033[0m  (ahead %s, behind %s)\n" "$marker" "$name" "$abar" "$bbar" "$ahead" "$behind"; done < "$fifo2"; rm -f "$fifo2"; }; f "$1"'</code></pre>
</details>
